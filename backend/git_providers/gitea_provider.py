...
class GiteaProvider(GitProvider):
    def __init__(self):
        ...
        self.base_url = (settings.gitea_base_url or "https://gitea.com").rstrip("/")
        self.webhook_secret = settings.gitea_webhook_secret
    ...
    async def get_pr_diff(self, owner: str, repo: str, number: int) -> str:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            resp = await client.get(
                f"{self.base_url}/api/v1/repos/{owner}/{repo}/pulls/{number}.diff",
                headers=headers
            )
    ...
    async def get_pr_files(self, owner: str, repo: str, number: int) -> list[dict]:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            resp = await client.get(
                f"{self.base_url}/api/v1/repos/{owner}/{repo}/pulls/{number}/files",
                headers=headers
            )
            resp.raise_for_status()
            return resp.json()
    ...
    async def post_pr_comment(self, owner: str, repo: str, number: int, body: str) -> None:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            await client.post(
                f"{self.base_url}/api/v1/repos/{owner}/{repo}/issues/{number}/comments",
                headers=headers,
                json={"body": body}
            )
    ...
    async def post_inline_comments(
        ...
        await self.post_pr_comment(
            owner, repo, number,
            "\n\n".join(self._format_comment(c) for c in comments)
        )
    ...
    async def add_labels(self, owner: str, repo: str, number: int, labels: list[str]) -> None:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            for label in labels:
                await client.post(
                    f"{self.base_url}/api/v1/repos/{owner}/{repo}/issues/{number}/labels",
                    headers=headers,
                    json={"labels": [label]}
                )
    ...
    async def health_check(self) -> bool:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                resp = await client.get(f"{self.base_url}/api/v1/user", headers=headers)
                return resp.status_code == 200
            except:
                return False

    def _format_comment(self, comment: ReviewComment) -> str:
        msg = f"**{comment.path}:{comment.line or '?'}** - {comment.severity.value.upper()} [{comment.category.value}]\n\n{comment.message}"
